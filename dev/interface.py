...
def imageClassifier(label, model, class_names):
    ...
    if label and label.image: 
        ...
        if (class_names[np.argmax(score)] == "shark" or class_names[np.argmax(score)] == "tiger" ):
            dangerous = True

        if dangerous:
            messagebox.showinfo(title="Classification", message= "This image is most likely a(n) {} with a {:.2f} percent confidence. This animal is dangerous."
            .format(class_names[np.argmax(score)], 100 * np.max(score)))
        else:
            messagebox.showinfo(title="Classification", message= "This image is most likely a(n) {} with a {:.2f} percent confidence. This animal is not dangerous"
            .format(class_names[np.argmax(score)], 100 * np.max(score)))
